...
public class PotionShopPage implements ShopData {
    ...
    public PotionShopPage() {
        ...
        shopItems.add(new ShopItemType(new ItemBuilder().setType(XMaterial.POTION.get()).setPotionData(new PotionEffect(Objects.requireNonNull(XPotion.JUMP_BOOST.get()), 600, 5)).setDisplayName("跳跃药水§7（30秒）").getItem(), "跳跃药水§7（30秒）", ColorType.NONE, new PriceCost(XMaterial.EMERALD.get(), 1, 100)));
