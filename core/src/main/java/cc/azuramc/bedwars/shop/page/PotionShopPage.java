...
public class PotionShopPage implements ShopData {
    ...
    public PotionShopPage() {
        ...
        shopItems.add(new ShopItemType(new ItemBuilder().setType(XMaterial.POTION.get()).setPotionData(new PotionEffect(Objects.requireNonNull(XPotion.JUMP_BOOST.get()), 900, 5)).setDisplayName("§b跳跃药水§7（45秒）").getItem(), "跳跃药水§7（45秒）", ColorType.NONE, new PriceCost(XMaterial.EMERALD.get(), 1, 100)));
