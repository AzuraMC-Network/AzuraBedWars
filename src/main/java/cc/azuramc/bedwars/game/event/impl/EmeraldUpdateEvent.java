...
public class EmeraldUpdateEvent extends GameEvent {
    ...
    public void execute(GameManager gameManager) {
        // 获取绿宝石刷新任务
        cc.azuramc.bedwars.game.event.Runnable runnable = gameManager.getEventManager().getRunnables().get(EMERALD_REFRESH_KEY);
        if (runnable == null) {
            return; // 防止NPE
        }
        ...
        // 调整当前周期和未来周期的刷新时间
        updateRefreshTime(runnable, newRefreshSeconds);
    }
     ...
     */
    private void updateRefreshTime(Runnable runnable, int newRefreshSeconds) {
        // 如果当前倒计时比新设置的时间长，则直接调整为新时间
        if (runnable.getNextSeconds() > newRefreshSeconds) {
            runnable.setNextSeconds(newRefreshSeconds);
        }
        
        // 设置未来周期的刷新间隔
        runnable.setSeconds(newRefreshSeconds);
    }
}
