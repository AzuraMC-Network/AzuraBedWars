...
public class DefaultShopPage implements ShopData {
    ...
    public DefaultShopPage() {
        ...
        shopItems.add(new ShopItemType(new ItemBuilder().setType(XMaterial.POTION.get()).setPotionData(new PotionEffect(Objects.requireNonNull(PotionEffectWrapper.SPEED()), 600, 1)).setDisplayName("§b速度药水§7（30秒）").getItem(), "速度药水§7（30秒）", ColorType.NONE, new PriceCost(XMaterial.EMERALD.get(), 1, 100)));
        ...
        shopItems.add(new ShopItemType(new ItemBuilder().setType(XMaterial.POTION.get()).setPotionData(new PotionEffect(Objects.requireNonNull(PotionEffectWrapper.JUMP_BOOST()), 600, 1)).setDisplayName("跳跃药水§7（30秒）").getItem(), "跳跃药水§7（30秒）", ColorType.NONE, new PriceCost(XMaterial.EMERALD.get(), 1, 100)));
