...
import cc.azuramc.bedwars.database.map.IMapStorage;
...
public class MapCommand {
    ...
    public void getMapCommand(BukkitCommandActor actor) {
        CommandUtil.sendLayout(actor, CC.CHAT_BAR);
        CommandUtil.sendLayout(actor, CC.color("&b&lAzuraBedWars &8- &7v" + plugin.getDescription().getVersion() + " &8- &b起床战争 - 地图设置"));
        CommandUtil.sendLayout(actor, "");
        CommandUtil.sendLayout(actor, CC.color("&7 • &f/map create <mapName> &7创建新的地图"));
        CommandUtil.sendLayout(actor, CC.color("&7 • &f/map setWaiting <mapName> &7设置等待大厅位置"));
        CommandUtil.sendLayout(actor, CC.color("&7 • &f/map setAuthor <mapName> <authorName> &7设置地图作者名"));
        CommandUtil.sendLayout(actor, CC.color("&7 • &f/map setTeamPlayers <mapName> <number> &7设置队伍最大人数"));
        CommandUtil.sendLayout(actor, CC.color("&7 • &f/map setMinPlayers <mapName> <number> &7设置地图最小需要人数"));
        CommandUtil.sendLayout(actor, CC.color("&7 • &f/map setRespawn <mapName> &7设置地图重生点"));
        CommandUtil.sendLayout(actor, CC.color("&7 • &f/map addBase <mapName> &7增加基地出生点"));
        CommandUtil.sendLayout(actor, CC.color("&7 • &f/map addDrop <mapName> <type> &7增加资源点 (类型: BASE/DIAMOND/EMERALD)"));
        CommandUtil.sendLayout(actor, CC.color("&7 • &f/map addShop <mapName> <type> &7增加商店 (类型: ITEM/UPGRADE)"));
        CommandUtil.sendLayout(actor, CC.color("&7 • &f/map setPos1 <mapName> &7设置地图边界1"));
        CommandUtil.sendLayout(actor, CC.color("&7 • &f/map setPos2 <mapName> &7设置地图边界2"));
        CommandUtil.sendLayout(actor, CC.color("&7 • &f/map save <mapName> &7保存地图"));
        CommandUtil.sendLayout(actor, CC.color("&7 • &f/map load <mapName> &7加载地图配置"));
        CommandUtil.sendLayout(actor, "");
        CommandUtil.sendLayout(actor, CC.color("&7 • &f/map list <type> &7查看指定存储方式地图列表"));
        CommandUtil.sendLayout(actor, CC.color("&7 • &f/map migrate &7迁移所有地图数据存储方式 (类型: JSON/MYSQL)"));
        CommandUtil.sendLayout(actor, CC.color("&7 • &f/map migrate <源类型> <目标类型> [mapName] &7迁移地图存储方式"));
        CommandUtil.sendLayout(actor, CC.color("&7 • &f/map info <mapName> &7查看地图信息"));
        CommandUtil.sendLayout(actor, CC.CHAT_BAR);
    }
    ...
    public void createMap(BukkitCommandActor actor, String mapName) {
        ...
        CommandUtil.sendLayout(actor, CC.color("&a地图创建成功!"));
    }
    ...
    public void setAuthor(BukkitCommandActor actor, String mapName, String authorName) {
        ...
        CommandUtil.sendLayout(actor, CC.color("&a地图作者设置成功!"));
    }
    ...
    public void setTeamPlayers(BukkitCommandActor actor, String mapName, int value) {
        ...
        CommandUtil.sendLayout(actor, CC.color("&a队伍最大人数设置成功!"));
    }
    ...
    public void setMinPlayers(BukkitCommandActor actor, String mapName, int value) {
        ...
        CommandUtil.sendLayout(actor, CC.color("&a地图最小人数置成功!"));
    }
    ...
    public void list(BukkitCommandActor actor, String type) {
        ...
        try {
            ...
            CommandUtil.sendLayout(actor, CC.color("&c无效的存储类型: " + type));
        }

        IMapStorage storage = MapStorageFactory.getStorage(storageType);
        List<String> mapNames = storage.getAllMapNames();

        CommandUtil.sendLayout(actor, CC.color("&a存储类型 [" + storageType.name() + "] 中的地图列表 (" + mapNames.size() + "):"));

        if (mapNames.isEmpty()) {
            CommandUtil.sendLayout(actor, CC.color("&e没有找到地图"));
        } else {
    ...
    public void info(BukkitCommandActor actor, String mapName) {
        ...
        CommandUtil.sendLayout(actor, CC.CHAT_BAR);
        CommandUtil.sendLayout(actor, CC.color("&b地图信息"));
        CommandUtil.sendLayout(actor, " ");
        CommandUtil.sendLayout(actor, CC.color(" &9&l▸ &f名称: &b" + mapData.getName()));
        CommandUtil.sendLayout(actor, CC.color(" &9&l▸ &f作者: &b" + mapData.getAuthor()));
        CommandUtil.sendLayout(actor, CC.color(" &9&l▸ &f地图最小人数: &b" + mapData.getPlayers().getMin()));
        CommandUtil.sendLayout(actor, CC.color(" &9&l▸ &f队伍最大人数: &b" + mapData.getPlayers().getTeam()));
        CommandUtil.sendLayout(actor, " ");
        CommandUtil.sendLayout(actor, CC.color(" &9&l▸ &fPos1: &b" + mapData.getPos1Location()));
        CommandUtil.sendLayout(actor, CC.color(" &9&l▸ &fPos2: &b" + mapData.getPos2Location()));
        CommandUtil.sendLayout(actor, CC.color(" &9&l▸ &f大厅位置: &b" + mapData.getWaitingLocation()));
        CommandUtil.sendLayout(actor, CC.color(" &9&l▸ &f基地出生点: &b" + mapData.getBases()));
        CommandUtil.sendLayout(actor, CC.color(" &9&l▸ &f基地资源点: &b" + mapData.getDrops(MapData.DropType.BASE)));
        CommandUtil.sendLayout(actor, CC.color(" &9&l▸ &f钻石资源点: &b" + mapData.getDrops(MapData.DropType.DIAMOND)));
        CommandUtil.sendLayout(actor, CC.color(" &9&l▸ &f绿宝石资源点: &b" + mapData.getDrops(MapData.DropType.EMERALD)));
        CommandUtil.sendLayout(actor, CC.CHAT_BAR);
    }
    ...
    public void save(BukkitCommandActor actor, String mapName) {
        if (mapManager.saveMapData(mapName)) {
            CommandUtil.sendLayout(actor, CC.color("&a保存成功!"));
            return;
        }
        CommandUtil.sendLayout(actor, CC.color("&c地图保存失败!"));
    }

    @Subcommand("load")
    public void load(BukkitCommandActor actor, String mapName) {
        mapManager.getAndLoadMapData(mapName);
        CommandUtil.sendLayout(actor, CC.color("&a地图加载成功!"));
    }
}
