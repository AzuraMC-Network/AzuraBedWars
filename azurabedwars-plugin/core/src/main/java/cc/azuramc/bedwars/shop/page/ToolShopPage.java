...
public class ToolShopPage implements ShopData {
    ...
    public ToolShopPage() {
        ...
        shopItems.add(new ShopItemType(new ItemBuilder().setType(XMaterial.WOODEN_PICKAXE.get()).setUnbreakable(true, true).addItemFlag(ItemFlag.HIDE_ATTRIBUTES).addEnchant(XEnchantment.EFFICIENCY.get(), 1).getItem(), "稿子", ColorType.PICKAXE, new PriceCost(XMaterial.IRON_INGOT.get(), 10, 10)));
        shopItems.add(new ShopItemType(new ItemBuilder().setType(XMaterial.WOODEN_AXE.get()).setUnbreakable(true, true).addItemFlag(ItemFlag.HIDE_ATTRIBUTES).addEnchant(XEnchantment.EFFICIENCY.get(), 1).getItem(), "斧子", ColorType.AXE, new PriceCost(XMaterial.IRON_INGOT.get(), 10, 10)));
