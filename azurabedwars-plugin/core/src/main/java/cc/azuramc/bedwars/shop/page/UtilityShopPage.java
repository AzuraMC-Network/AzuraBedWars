...
//        shopItems.add(new ShopItemType(new ItemBuilder().setType(XMaterial.RED_BED.get()).setDisplayName("§e回春床").setLores(" ", "§f开局10分钟内可恢复一次床").getItem(), "回春床", ColorType.NONE, new PriceCost(XMaterial.EMERALD.get(), 1, 80)));
        ...
        shopItems.add(new ShopItemType(new ItemBuilder().setType(XMaterial.WOLF_SPAWN_EGG.get()).setSpawnEggType(EntityType.WOLF).getItem(), "梦幻守护者", ColorType.NONE, new PriceCost(XMaterial.IRON_INGOT.get(), 128, 128)));
        ...
        shopItems.add(new ShopItemType(new ItemBuilder().setType(XMaterial.MILK_BUCKET.get()).getItem(), "魔法牛奶", ColorType.NONE, new PriceCost(XMaterial.GOLD_INGOT.get(), 8, 80)));
    }
}
