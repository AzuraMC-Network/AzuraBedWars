...
public class ArmorShopPage implements ShopData {
    ...
    public ArmorShopPage() {
        mainShopItem = new ShopItemType(new ItemBuilder().setType(XMaterial.CHAINMAIL_BOOTS.get()).setLores("§e点击查看！").addItemFlag(ItemFlag.HIDE_ATTRIBUTES).getItem(), "§a盔甲", ColorType.NONE, null);
        ...
        shopItems.add(new ShopItemType(new ItemBuilder().setType(XMaterial.DIAMOND_BOOTS.get()).setUnbreakable(true, true).addItemFlag(ItemFlag.HIDE_ATTRIBUTES).getItem(), "钻石装备§7（死亡不掉落）", ColorType.NONE, new PriceCost(XMaterial.EMERALD.get(), 6, 600)));
    }
